...
Status GetElem_L(LinkList &L, int i, ElemType &e)//查找,找到第i个元素，放到e里面,返回位序
{
    int j = 1;
    LinkList p = L->next;
    while (p != NULL && j < i)
...
int LocateElem_L(LinkList L, ElemType e)//查找值
{
    LinkList p = L->next;
    int i = 1;
...
Status ListDelete_L(LinkList &L, int i, ElemType &e)//删除指定结点
{
    int j = 1;
    LinkList p = L->next, q;
    while (p != NULL && j < i - 1)
