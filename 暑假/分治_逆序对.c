/**
问题描述：设?[1, ? , ?]是一个包含?个不同非负整数的数组。如果在? < ?的情况下， 有? ? > ?[?]，
则（? ? ，?[?]）就称为A中的一个逆序对。例如，数组（3，1，4， 5，2）的“逆序对”有<3,1>、<3,2>、<4,2>、<5,2>共4个。
输入：每个测试用例包括两行，第一行输入整数的个数n，? ≤ 10000，第二行输入
?个整数，数与数之间用空格隔开。最后一行输入-1，表示输入结束。
